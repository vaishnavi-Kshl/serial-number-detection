...
from serial_agent.api import _guide_response
...
from serial_agent.repository import InMemorySerialRepository
...
def test_repository_upsert_splits_full_document_into_b1_and_b2():
    ...
    assert repo.get_b1("asset-2").serial_number_series == "SN2000-SN2999"
    assert repo.get_b1("asset-2").serial_number_location is None

    assert repo.get_b2("asset-2") is not None
    assert repo.get_b2("asset-2").serial_number_location == "Inside the rear panel"
    assert repo.get_b2("asset-2").serial_number_series is None
...
def test_ingest_pdf_upserts_each_page_separately(monkeypatch):
    ...
    assert repo.get_b2("page-1") is not None
    assert repo.get_b2("page-2") is not None
    assert repo.get_b2("page-1").document_id == "doc-1"
    assert repo.get_b2("page-2").document_id == "doc-1"
...
def test_verify_image_returns_extracted_serial_even_without_b1(monkeypatch):
    repo = InMemorySerialRepository()
    repo.upsert_b2(
        AssetRecord(
...
def test_verify_image_returns_short_failure_when_serial_missing(monkeypatch):
    ...
    assert result.message == "No matching asset found in Qdrant set b1. Add the serial number series first."
